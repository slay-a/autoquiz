...
import {
  BookOpen, Layers, PlusCircle, Users, ChevronRight,
  Loader2, Hash, LogIn, Trophy
} from "lucide-react";
...
export default function StudentDashboard() {
  ...
  async function fetchAll() {
    ...
    const [
      ...
      supabase.from("class_members").select("class_id, classes(id, name, class_code, saved_quizzes(*))").eq("student_id", user.id),
    ]);
    ...
    const sharedQs = joinedClasses.flatMap((c) => (c.saved_quizzes ?? []).map((q) => ({ ...q, className: c.name })));
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm">Your study hub — quizzes, flashcards, classes.</p>
        </div>
      ...
      {loading ? (
        ...
        <>
          {tab === "quizzes" && (
            <QuizList quizzes={myQuizzes} emptyMsg="You haven't generated any quizzes yet." />
          )}
          {tab === "class" && (
            <QuizList quizzes={classQuizzes} emptyMsg="No quizzes shared by instructors yet." showClass />
          )}
          {tab === "flashcards" && (
            <FlashcardList sets={flashcardSets} />
          )}
          {tab === "classes" && (
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {classes.map((c) => (
        <div key={c.id} className="card p-5">
          ...
          <p className="text-xs text-gray-400">{c.saved_quizzes?.length ?? 0} shared quizzes</p>
        </div>
      ))}
    </div>
  );
