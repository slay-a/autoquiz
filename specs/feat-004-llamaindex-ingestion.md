# Feature Brief: LlamaIndex Ingestion Refactor
...
**Acceptance Criteria:**
- [ ] AC-1: `backend/app/utils/parsers.py` uses `llama_index.readers.file` readers (PDFReader, DocxReader, PptxReader) to extract content from uploaded files.
- [ ] AC-2: Each reader returns a list of LlamaIndex `Document` objects with `page_label` (or equivalent) populated in node metadata.
- [ ] AC-3: `.pdf`, `.docx`, and `.pptx` extensions are still the only accepted file types; all others raise a `ValueError` with the `extract|` stage prefix.
- [ ] AC-4: The old custom `parse_pdf`, `parse_docx`, `parse_pptx` functions and `fitz`/`docx`/`pptx` direct imports are removed from `parsers.py`.
...
**Acceptance Criteria:**
- [ ] AC-1: `backend/app/services/ingestion.py` creates a `SentenceSplitter` with `chunk_size=settings.chunk_size_tokens` and `chunk_overlap=settings.chunk_overlap_tokens`.
- [ ] AC-2: `SentenceSplitter.get_nodes_from_documents()` replaces the custom `clean_text`, `detect_sections`, and `chunk_sections` functions.
- [ ] AC-3: The custom `clean_text`, `detect_sections`, and `chunk_sections` functions are removed from `ingestion.py`.
- [ ] AC-4: Each resulting `TextNode` has non-empty `text`.
...
**Acceptance Criteria:**
- [ ] AC-1: `ingest_document` returns a list of dicts with keys `chunk_id`, `file_id`, `section_id`, `section_title`, `page_numbers`, `text` — identical shape to the existing contract.
- [ ] AC-2: `chunk_id` is a new `uuid4` string per node.
- [ ] AC-3: `page_numbers` is populated from `node.metadata.get("page_label")` if present, defaulting to `[1]` if absent.
- [ ] AC-4: `section_title` is populated from `node.metadata.get("section_title")` if present, defaulting to `None`.
- [ ] AC-5: No LlamaIndex `VectorStoreIndex`, `StorageContext`, or `SupabaseVectorStore` classes are used — storage is written directly to the `chunks` table by the Celery worker.
...
| Student | — | Upload | Same |
| Unauthenticated | None | All | FastAPI auth dependency |
...
- LlamaIndex embeddings pipeline (`OpenAIEmbedding` via LlamaIndex) is not used — embedding calls remain in the Celery worker using the OpenAI client directly.
...
- [x] Design decisions are filled in
